...
/*:
 ...
 - 注意: この配列の大きさを知るシンタックスは関数ではありません。よく見ると()がついていませんね！`count`は_プロパティ_ つまり配列に紐づいている値です。ドットを使うことでこの値を取得することができます。
...
/*:
 ...
 - callout(重要): `...`を使う時は少し注意しましょう。配列の位置の番号は0から(配列の`count` - 1)の間です。下のコードはエラーが出てしまいます。なぜでしょう？
