/*:
 ...
 コンウェイのライフゲームシミュレーションを作るために、まずは"palette"と呼ばれるものを用意します。これは、ユーザーが色々な`Characters`（文字）をこのシミュレーションで使うことができるようにするためのものです。まずは”一次元”配列を使います。
...
/*:
 ...
 - note: (メモ)"String(文字列)"は"Character(文字)"からできています。"Character(文字)"は、"String(文字列)"の一要素で、２文字以上を保存することはできません。また、配列はどんなデータ型も保存することができます。（ただし配列に入っている全てのデータは同じデータ型でないといけません。）
 ...
 - important:（重要）`control + command + space`を押すと絵文字キーボードが開きます。
...
配列から要素を取り出すためには、上の例のように配列の名前の後に取り出したい要素の番号を付け加えます。
 ...
 - important:（重要）配列の番号は、0から始まります！つまり、`palette[1]`は、_2_つ目の要素を取り出すということになります。
...
/*:
 ...
 - callout(試してみよう): _添字_ を使うと、配列から要素を取り出すだけではなく、要素を定義することができます。これまでに学んだ定義の方法ととても似ています。下のコードを実際にコーディングエリアに書いてみましょう。
...
/*:
 ...
 - important:（重要）paletteは、`let`を使って定義しました。つまり、この値は変更ができない配列ということです。もし、paletteの値を後から変更可能にしたければ、`let`ではなく`var`を使って定義します。
...
/*:
 ...
 - note:（メモ）`count`を使うと、配列の大きさを知ることができます。
...
/*:
 ...
 - important:（重要）この配列の大きさを知るシンタックスは関数ではありません。よく見ると()がついていませんね！`count`は_プロパティ_ つまり配列に紐づいている値です。ドットを使うことでこの値を取得することができます。
...
/*:
 ...
 - important:（重要）`...`を使う時は少し注意しましょう。配列の位置の番号は0から(配列の`count` - 1)の間です。下のコードはエラーが出てしまいます。なぜでしょう？
 ...
 - note:（メモ）この新しいやり方はこのチュートリアルでは使いませんが、知っておくと後々便利です。
