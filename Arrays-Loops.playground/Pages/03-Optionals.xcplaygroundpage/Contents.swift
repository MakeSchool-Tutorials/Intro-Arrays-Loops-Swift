/*:
 ...
 - メモ: `nil`はよく他のプログラミング言語の`null`と同じように言われます。Swiftは他の言語と比べて値が空であることにすごく厳しい言語で、これによってバグを防ぐのに役立っています。

 ## nil

　_オプショナル_として定義していれば、Swiftのどんな値も`nil`をセットすることができます。
...
/*:
 ...
 - 実験: 上で定義した`maybeAnInt`の値を`nil`から別の値に変えてみましょう。プリントされる結果が変わるはずです。
...
/*:
 ...
 - 実験: `maybeAnInt`を何度か変えてみて、もし`maybeAnInt`に値が入っていれば、 `definitelyAnInt`が同じ値になるかどうかを実験してみましょう。もし値が入っていなければ、`definitelyAnInt`は作られません。

 - 重要: Unwrapする時、新しく作る値に同じ変数名を使うことが慣習になっています。この値はif文の中ではunwrapされた状態ですが、if文の外ではオプショナルに戻ります。
...
/*:
 ...
 - 実験:　上の行をアンコメントして何が起こるか見てみましょう。空の`alwaysAString`にアクセスしようとするため、クラッシュしてしまいます。
