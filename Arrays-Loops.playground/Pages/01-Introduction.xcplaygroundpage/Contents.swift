...
配列はデータの集まりを操作するために必要なものです。これまで、一つ一つの値を変数や定数に保存することは学びましたね。（'let'や'var'を使いました。）では、もしたくさんのデータを保存したかったら、どうすれば良いでしょうか。一つ一つに変数や定数を定義していたら大変ですね。
 ...
 このチュートリアルでは、配列を作る方法、その中のデータを"forループ"というものを使って操作する方法を学びます。その知識を使って、「ライフゲーム」と呼ばれる有名なシミュレーションをコードで実装していきます！
...
いや、これではないですよ！
 ...
 コンウェイのライフゲーム（Conway's Game of Life）は実はいわゆるゲームとは少し違います。これは[_セル・オートマトン_](https://ja.wikipedia.org/wiki/%E3%82%BB%E3%83%AB%E3%83%BB%E3%82%AA%E3%83%BC%E3%83%88%E3%83%9E%E3%83%88%E3%83%B3)と呼ばれる、２次元のグリッドにセルの誕生や進化、淘汰を表現するシミュレーションなのです。セルが塗りつぶされたり、白に戻ったりします。まるで微生物やバクテリアが広がるように、条件によって黒のセルが増えたり、減ったりします。
