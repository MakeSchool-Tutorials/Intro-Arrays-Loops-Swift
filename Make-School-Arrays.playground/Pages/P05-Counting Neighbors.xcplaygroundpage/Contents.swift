...
/*:
Before we do our neighbor counting logic, let's instead prepare for ourselves a helper function – something that will help us simplify our logic when we write a `countNeighbors` function later on. Here, we have a `getAlive` function, which takes a `map` of `[[Character?]]`, and `x` and `y` coordinates for the cell in question.

Given these parameters, you should:

- Check if the coordinates `x` and `y` are _in bounds_ of `map`. If not, return 0.
- Get the `Character?` value from map. Is it equal (`==`) to `nil`? If so, return 0.
- If all checks failed, then return 1.

Hint: To get the number of columns in map, you can use `map.count`. To get the number of rows, you can get the size of the first column: `map[0].count`
...
/*:
Great! Now, we have a `countNeighbors` function, in which you need to write code that, given a `map` of `[[Character?]]`, and `x` and `y` coordinates, return as an `Int` how many neighbors the cell has.
